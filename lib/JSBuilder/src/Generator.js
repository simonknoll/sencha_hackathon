...
Ext.generator.Base = Ext.extend(Object, {
    ...
    template: function(name, data, filename) {
        ...
        var name        = './lib/JSBuilder/src/generators/' + this.dirName + '/templates/' + name + '.js',
    ...
    copyDir: function(dirName, destination) {
        ...
        if (!this.pretend) {
            Filesystem.copy(dirName + (Filesystem.endsWith(dirName, Filesystem.sep) ? '' : Filesystem.sep), destination + (Filesystem.endsWith(destination, Filesystem.sep) ? '' : Filesystem.sep));
        }
    },
